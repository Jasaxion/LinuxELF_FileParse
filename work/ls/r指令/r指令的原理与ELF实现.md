...
<img src="r%E6%8C%87%E4%BB%A4%E7%9A%84%E5%8E%9F%E7%90%86%E4%B8%8EELF%E5%AE%9E%E7%8E%B0.assets/image-20230624094924381.png" alt="image-20230624094924381" style="zoom:47%;" />

readelf命令用于查看和分析可执行文件、共享目标文件和目标文件的内部结构和信息。在这个特定的readelf命令输出中，我们使用了`-r`选项，该选项显示了重定位节的信息。
...
定位节包含了15个条目，每个条目对应一个需要进行重定位的位置。其中：
...
这些重定位节中的条目指示了需要在程序加载和执行时进行地址重定位的位置和相关符号的信息。重定位的过程将在程序加载时由链接器完成，以确保程序中引用的符号能够正确地解析和链接到实际的内存地址上。这对于程序的正确执行至关重要。
